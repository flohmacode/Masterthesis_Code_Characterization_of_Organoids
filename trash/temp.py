...
def rbf_kernel(t1, t2, lengthscale, sigma_sq):
    """
    Computes the Squared Exponential (RBF) covariance matrix for a Gaussian Process (GP).

    Parameters:
    - t1 (numpy.ndarray): First set of time points (e.g., observation times).
    - t2 (numpy.ndarray): Second set of time points (e.g., observation times).
    - lengthscale (float): Length scale parameter of the RBF kernel (controls smoothness).
    - sigma_sq (float): Signal variance parameter of the RBF kernel (controls amplitude).

    Returns:
    - numpy.ndarray: Covariance matrix of shape (len(t1), len(t2)).

    Notes:
    - The RBF kernel is defined as: `K(t1, t2) = sigma_sq * exp(-(t1 - t2)^2 / (2 * lengthscale^2))`.
    - This kernel is commonly used in Gaussian Processes for smooth, infinitely differentiable functions.
    """
