...
fig_temp, ax_temp, img_mat = mr_imaging.get_reconstructed_img(study_directory_img, scan_img_number, slice_idx_img, 'sagital')
plt.close()

header_img = file_utils.read_bruker_all_headers(study_directory_img, scan_img_number)
fov_img = header_img['PVM_Fov']

header_spec = file_utils.read_bruker_all_headers(study_directory, scan_no = experimentindice[2])
...
fig, ax = csiplots.csi_overlay_sagittal(
    ...
    offset=offset,
    figsize = (8,8),
    title = 'Chemical Shift Image coronal'
)
plt.savefig('./fig_csi/sagital_nmr')
plt.show()